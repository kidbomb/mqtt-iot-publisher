...
class ZigbeeNode extends Device {
  ...
  constructor(adapter, id64, id16) {
    ...
    super(adapter.listener, adapter, deviceId);
    ...
    if (frame.zcl) {
      ...
      if (frame.zcl.frameCntl.disDefaultRsp == 0 &&
        ...
        const defaultRspFrame =
          this.makeDefaultRspFrame(frame, STATUS.SUCCESS);
  ...
  makeEnrollRspFrame(enrollReqFrame, status, zoneId) {
    ...
    if (enrollReqFrame) {
      ...
      sourceEndpoint =
        this.findZhaEndpointWithInputClusterIdHex(CLUSTER_ID.SSIASZONE_HEX);
