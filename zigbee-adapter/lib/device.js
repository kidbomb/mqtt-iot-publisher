...
class Device {
  constructor(listener, adapter, id) {
    if (typeof id !== 'string') {
      id = id.toString();
    }
  ...
  notifyPropertyChanged(property) {
	  this.listener.propertyChanged(property);
  }

  actionNotify(action) {
	  console.log('ADAPTER: action notify')
  }

  eventNotify(event) {
	  console.log('ADAPTER: event notify')
  }

  connectedNotify(connected) {
	  console.log('ADAPTER: connected notify')
  }
